$(document).ready(function () {
    ...
    function getBoosts(startIndex, max, scrollToTop, old, shouldPew) {
        const lastIndex = $('div.outgoing_msg:first').data('msgid') ?? null;
        const boostIndex = startIndex ?? lastIndex;
        const noIndex = !Number.isInteger(boostIndex);
    ...
    function renderBoosts(data, max, scrollToTop, shouldPew) {
        data.forEach((element, index) => {
            if (messageIds.includes(element.index)) {
        ...
        if (isMore && $('div.outgoing_msg').length > 0 && $('div.loadmore').length == 0) {
